...
Handles datasets with missing values (heart_disease) via a SimpleImputer
...
def _build_preprocessing_pipeline(
    has_missing_values: bool, scale_features: bool, force_imputation: bool
) -> Pipeline:
    steps = []
    if has_missing_values or force_imputation:
    ...
    if scale_features:
        ...
        # Keep one shared transform/fit path even when the model does not need scaling.
...
def get_prepared_splits(
    ...
    ``scale_features`` is enabled for Logistic Regression and disabled for
    Random Forest. In both cases, any required imputer and transform are fit
    only on ``X_train`` and reused unchanged for validation and test data.
    ``force_imputation`` enables the same training-fitted imputer for complete
    datasets so deterministic missingness stress tests can use the frozen
    training preprocessing without refitting on shifted test data.
    ...
    pipeline = _build_preprocessing_pipeline(
        bundle.has_missing_values, scale_features, force_imputation
    )
